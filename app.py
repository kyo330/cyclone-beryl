...
def load_csvs(file_objs):
    ...
    df["polarity"] = np.where(df.get("peakcurrent", pd.Series([np.nan]*len(df))) >= 0, "Positive", "Negative")
...
with c4:
    # minute/hour/day “frame” selector for scrub playback
...
frame_idx = 0
if frames:
    frame_idx = st.slider("Frame", 0, max(0, len(frames)-1), value=0, step=1,
...
if "peakcurrent" in frame_view.columns:
    size = np.clip(np.log1p(np.abs(frame_view["peakcurrent"])).fillna(0), 1, None)
else:
...
class_series = frame_view["class"].astype(str)
color = class_series.map(color_map).apply(lambda v: v if isinstance(v, list) else default_color)
...
if map_mode == "Points":
    ...
    # Hexagon density (nice HLMA-style density look)
    layers.append(
        pdk.Layer(
            "HexagonLayer",
            data=frame_view,
    ...
    # Extrude by icheight (scaled to km). Fallback for missing heights.
    height_km = (frame_view.get("icheight", pd.Series([0]*len(frame_view))) / 1000.0).fillna(0.0)
    scaled_h = (height_km.replace([np.inf, -np.inf], np.nan).fillna(0.0) * 10000.0).clip(0, 20000)  # scale for visibility
    layers.append(
        pdk.Layer(
            "ColumnLayer",
...
with st.sidebar:
    st.header("Data")
    st.write(
        "Upload one or more CSVs exported from ENTLN "
        "(columns like `timestamp, latitude, longitude, peakcurrent, icheight, type`)."
    )
    files = st.file_uploader("CSV files", type=["csv"], accept_multiple_files=True)
...
# --- Time slider (patched to avoid tz errors) ---
def _naive(dt_ts):
    # Convert UTC tz-aware pandas.Timestamp to naive datetime for Streamlit slider
    if hasattr(dt_ts, "tz") and dt_ts.tz is not None:
        return dt_ts.tz_convert("UTC").tz_localize(None).to_pydatetime()
    return pd.Timestamp(dt_ts).to_pydatetime()
...
with c4:
    # Future: add sensor count or ellipse axes filters here if needed
    st.markdown("")
