...
with col1:
    upl_points = st.file_uploader(
        ...
        type=["csv"],
        help="Expected columns (aliases OK): lat/latitude, lon/longitude, alt/altitude(_m). Optional time/timestamp/datetime."
    )
with col2:
    upl_winds = st.file_uploader(
        "Upload Storm/Wind Reports CSV (Lat, Lon[, Comments])",
        type=["csv"],
        help="Expected columns (aliases OK): Lat/Latitude, Lon/Longitude. Optional Comments/Remark/Description."
    )
...
# We inject JSON with .replace("__POINTS__", ...) and .replace("__WINDS__", ...)
html_tpl = """
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
  ...
  <script>
    ...
    // Prefer injected data; else fall back to GitHub CSVs
    ...
    function setAutoRefresh(enabled, seconds){
      ...
      // With local uploads, this only re-runs the same in-memory data; kept for fallback
...
html_final = html_tpl.replace("__POINTS__", points_js).replace("__WINDS__", winds_js)
